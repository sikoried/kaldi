...
#  
#  Modifications: (c) 2015 Remeeting, Inc.  Korbinian Riedhammer
#
...
# koried: removed reverse mapping
getopts("hrt") || usage();
usage() if $opt_h;
...
sub TagToTextStartTag {
  ...
  if ($elem eq "segment") {
    ...
    # koried:  not need for ASR setup

    # add("..");
  } elsif ($elem eq "VocalSound") {
    # koried: we're interested in cough and laugh
    if (exists($attr{'Description'})) {
      # make them single words!
      $attr{'Description'} =~ tr/ -/_/;
      $TagText .= ("(" . uc($attr{'Description'}) . ")");
    } else {
      add("(SPN)");
    }
    # add("{VOC");
    ...
    addif("(NOISE)", 'Description', %attr);
    #add("{NVC");
    #addif(" %s", 'Description', %attr);
    #add("}");
  } elsif ($elem eq "Comment") {
    # koried:  no need for ASR setup
